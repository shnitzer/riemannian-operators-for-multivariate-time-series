...
imagesc(c_ldr);
title('LiDAR');
set(gcf,'Position',[1000,100,360,340]);
...
axis equal; axis tight; set(gcf,'Position',[900,250,305,335]);
figure
...
%%%%%%%%%%%%%
% saveas(gcf,'figures_for_paper\data1\Apos_psi_3_arrow.png')
% saveas(gcf,'figures_for_paper\data1\Apos_psi_3_arrow.fig')
% saveas(gcf,'figures_for_paper\data1\Apos_psi_3_arrow','epsc')
%%%%%%%%%%%%%%%%%%%%%
%% Correlations:
figure
...
Vsub1 = V_dm_ldr(:,g_rng_dm);
Vsub2 = V_dm_hps(:,g_rng_dm);

[~,SAB,~] = svd(Vsub1.'*Vsub2);
theta_sub = acos(diag(SAB));
...
evn      = 1;
vecs2use = {abs(VS(:,evn)), abs(VA_pos(:,evn)), abs(VA_neg(:,evn)), abs(V_dm_ldr(:,evn)), abs(V_dm_hps(:,evn))};

th_S   = 0.0147;    % multithresh(reshape(vecs2use{1},size(c_ldr)));
th_A1  = 0.007;     % multithresh(reshape(vecs2use{2},size(c_ldr)));
th_A2  = 0.014;     % multithresh(reshape(vecs2use{3},size(c_ldr)));
th_ldr = 0.007;      % multithresh(reshape(vecs2use{4},size(c_ldr)));
th_hps = 0.014;     % multithresh(reshape(vecs2use{5},size(c_ldr)));

nbins  = 100;

figure(500)
subplot(2,3,1); histogram(vecs2use{1},nbins);   hold on; scatter(th_S,0,100,'k','fill');    hold off; title('S');
subplot(2,3,2); histogram(vecs2use{2},nbins);   hold on; scatter(th_A1,0,100,'k','fill');   hold off; title('A+');
subplot(2,3,3); histogram(vecs2use{3},nbins);   hold on; scatter(th_A2,0,100,'k','fill');   hold off; title('A-');
subplot(2,3,5); histogram(vecs2use{4},nbins);   hold on; scatter(th_ldr,0,100,'k','fill');  hold off; title('LDR');
subplot(2,3,6); histogram(vecs2use{5},nbins);   hold on; scatter(th_hps,0,100,'k','fill');  hold off; title('HSI');

figure(501)
subplot(2,3,1); imagesc(reshape(vecs2use{1}.*(vecs2use{1}>th_S)   ,size(c_ldr))); colorbar; title('S');
subplot(2,3,2); imagesc(reshape(vecs2use{2}.*(vecs2use{2}>th_A1)  ,size(c_ldr))); colorbar; title('A+');
subplot(2,3,3); imagesc(reshape(vecs2use{3}.*(vecs2use{3}>th_A2)  ,size(c_ldr))); colorbar; title('A-');
subplot(2,3,5); imagesc(reshape(vecs2use{4}.*(vecs2use{4}>th_ldr) ,size(c_ldr))); colorbar; title('LDR');
subplot(2,3,6); imagesc(reshape(vecs2use{5}.*(vecs2use{5}>th_hps) ,size(c_ldr))); colorbar; title('HSI');
...
idx_S  = kmeans([VS(:,rng)*diag(dS(rng)),mean(mean(VS*diag(dS)))*xy_coord],k);
idx_A1 = kmeans([VA_pos(:,rng)*diag(dA_pos(rng)),mean(mean(VA_pos*diag(dA_pos)))*xy_coord],k);
idx_A2 = kmeans([VA_neg(:,rng_A_neg)*diag(dA_neg(rng_A_neg)),mean(mean(VA_neg*diag(dA_neg)))*xy_coord],k);
idx_al = kmeans([VS(:,rng),VA_neg(:,rng_A_neg),VA_pos(:,rng)],k2,'Start','Cluster');

figure
subplot(141); imagesc(reshape(idx_S ,size(c_ldr)))
subplot(142); imagesc(reshape(idx_A1,size(c_ldr)))
subplot(143); imagesc(reshape(idx_A2,size(c_ldr)))
subplot(144); imagesc(reshape(idx_al,size(c_ldr)))


figure
imagesc(reshape(idx_S  ,size(c_ldr))); colorbar; title('$$\psi^{(\mathbf{S})}\ clusters$$','Interpreter','latex','FontSize',14);
axis equal; axis tight; set(gcf,'Position',[900,250,305,335]);
figure
imagesc(reshape(idx_A1  ,size(c_ldr))); colorbar; title('$$\psi^{(\mathbf{A}^+)}\ clusters$$','Interpreter','latex','FontSize',14);
axis equal; axis tight; set(gcf,'Position',[900,250,305,335]);
figure
imagesc(reshape(idx_A2  ,size(c_ldr))); colorbar; title('$$\psi^{(\mathbf{A}^-)}\ clusters$$','Interpreter','latex','FontSize',14);
axis equal; axis tight; set(gcf,'Position',[900,250,305,335]);
figure
imagesc(reshape(idx_al  ,size(c_ldr))); colorbar; title('$$[\psi^{(\mathbf{S})},\psi^{(\mathbf{A}^+)},\psi^{(\mathbf{A}^-)}]\ clusters$$','Interpreter','latex','FontSize',14);
axis equal; axis tight; set(gcf,'Position',[900,250,305,335]);
...
params_fus = params;
params_fus.k_modes = params.k_modes; %7
